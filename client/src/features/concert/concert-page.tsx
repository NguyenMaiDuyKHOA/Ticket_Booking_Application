...
export function ConcertPage() {
  ...
  useEffect(() => {
    ...
    getItems({ itemTypeSlug: "concert", page: 1, pageSize: 50 })
      .then((result) => {
        if (isMounted) {
    ...
    <>
      ...
      <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6 lg:px-8">
        <section className="mx-auto max-w-7xl">
          <h1 className="text-3xl font-black text-white">Concert</h1>
          ...
          {!isLoading && !errorMessage && items.length === 0 ? (
            <p className="mt-6 rounded-md bg-white px-3 py-2 text-sm font-bold text-neutral-500">No concert items in the database yet.</p>
          ) : null}
